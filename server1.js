...
app.post('/reservedweb', function (req, res) {
  ...
  var FetchTrainNo="SELECT * FROM PNRDB WHERE PNR='9649665944';";

   con.query(FetchTrainNo,function(err,row,fields){
     if(err)throw err;
     else{
      var TrainNo=row[0].Train_No;
      var Berth_No=row[0].Berth;
      var Source=row[0].Source;
      var Destination=row[0].Destination;
      console.log(TrainNo);
      request({url:'https://api.railwayapi.com/v2/route/train/12903/apikey/d33dznniov/',json: true}, function(err, response, json) {
        if (err) {
         throw err;
        }
...
app.post('/unreservedweb', function (req, res) {
  ...
  var date=dt.format('d/m/Y H:M:S');
  var FetchTrainNo="SELECT Train_No FROM PNRDB WHERE PNR='9649665944';";
